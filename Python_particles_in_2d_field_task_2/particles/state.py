...
def get_state_3(states, iteration, current_velocity_func, point_types, step,
                weights=None, desintegrator=None, **kwargs):
    """Возвращае текущее состояние по предыдущему"""
    state_prev = states[iteration - 1, :, :]
    xy_prev = state_prev[:, :2]
    velocity_prev = state_prev[:, 2:4]
    ...
    states_cur = np.hstack([xy_cur, velocity_cur, types])
    states[iteration, :, :] = states_cur
    states = desintegrator.run(states, iteration, point_types)
    return states
    ...
    def fragment(self, states, iteration, point_types):
        ...
        for point_ind in range(state.shape[0]):
            ...
            #     num_new_points = round(0.63 * hours + 5.12)
                ...
                num_new_points = 0
            if num_new_points > 0:
                self.counter[point_ind] = 0
                new_point_type = point_type * -1
                logger.debug('Тип %d создано %d частиц', new_point_type, num_new_points)
                ...
                new_particles.extend([new_points_state] * num_new_points)
                point_types.loc[new_point_type, 'number'] += num_new_points
        new_states = states
        if new_particles:
            num_iterations, num_points, vector_length = states.shape
            new_states = np.ones([num_iterations, num_points + len(new_particles), vector_length], dtype=float) * np.nan
            new_states[:iteration + 1, :num_points, :] = states[:iteration + 1, :, :]
            new_states[iteration, num_points:, :] = np.vstack(new_particles)
        return new_states
...
class Desintegrator(object):
    ...
    def desintegrate(self, states, iteration):
        for point_ind in range(states.shape[1]):
            days = self.counter[point_ind] / (24 * 3600)
            if days >= 3:
                logger.debug('Частица %d уничтожена', point_ind)
                states[iteration, point_ind, :4] = np.nan
                self.counter[point_ind] = 0.0
        return states

    def run(self, states, iteration, point_types):
        self.count_point_is_near_continent(states[iteration, :, 0:2])
        states = self.desintegrate(states, iteration)
        return states
